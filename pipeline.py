...
class MusicRecommender:
    ...
    def calculate_distance(self, sv, sa, tv, ta):
        return np.sqrt((sv - tv) ** 2 + (sa - ta) ** 2)
    ...
    def search_by_emotion(self, target_valence, target_arousal, genre=None, num_songs=5):
        ...
        candidates['distance'] = candidates.apply(
            lambda row: self.calculate_distance(row['valence'], row['arousal'], target_valence, target_arousal), axis=1
        )
    ...
    def recommend_playlist(self, current_valence, current_arousal,
        ...
        if gradual and playlist_length >= 3:
            ...
            songs = self.search_by_emotion(target_valence, target_arousal, genre=genre, num_songs=playlist_length)
...
class EmotionMusicSystem:
    ...
    def train_model(self, physio_dir, annot_dir, progress_cb=None):
        n = self.predictor.train_with_subject_normalization(physio_dir, annot_dir, progress_cb=progress_cb)
        self.trained = True
        return n
