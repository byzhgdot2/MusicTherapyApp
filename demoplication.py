...
def _fit_predictor():
    """Fit the EmotionPredictor's model/scaler/imputer so .predict() works."""
...
def _run_training():
    ...
    ScriptRunContext). Reports progress via TRAIN_PROG and writes the terminal
    state to TRAIN_LOG. The main thread writes 'running' BEFORE this starts."""
    try:
        # feature-extraction sweep across subjects
        for i in range(1, 31):
            time.sleep(0.18)
            with open(TRAIN_PROG, "w") as f:
                f.write(str(i / 30 * 0.8))
        # fit the model
        predictor, n = _fit_predictor()
